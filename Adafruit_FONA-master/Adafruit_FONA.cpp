...
/********* Sleep & Wake ********************************************/
boolean Adafruit_FONA::sleep() {
  return sendCheckReply(F("AT+CSCLK=2"), ok_reply);
}

boolean Adafruit_FONA::wake() {
  mySerial->write("AT");
  return sendCheckReply(F("AT+CSCLK=0"), ok_reply);
}
...
boolean Adafruit_FONA::HTTP_GET_start(char *url, char *header,
              uint16_t *status, uint16_t *datalen){
  if (! HTTP_setup(url, header))
    return false;
...
boolean Adafruit_FONA::HTTP_setup(char *url, char *header) {
  // Handle any pending
  HTTP_term();
  ...
  if (! HTTP_para(F("URL"), url))
    return false;
  if (! HTTP_para(F("USERDATA"), header))
    return false;
